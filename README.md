...
Read each test case carefully. Complete all tasks in sequence.
Show your terminal output / screenshots as proof of completion wherever mentioned.

🧩 Test Case 1 — Linux System Audit & User Management
📖 Scenario

You are a Linux system administrator onboarding new employees at a tech company. Complete the following tasks on your system.
...
Create two groups: developers and qa.
Verify both groups were successfully created
Display the complete list of groups on the system
Create the following users and assign each to their respective group:
Employee	Department
aman	developers
ritu	developers
karan	qa
Employee ritu has resigned:
Ensure she has no active sessions
Delete her account along with her home directory
Verify she no longer exists on the system
✅ Expected Outcome
...
Users aman and karan are created and assigned correctly (verified via id command)

After deletion, any attempt to look up ritu returns:
...
A web server requires a properly configured index.html file with specific ownership and access permissions.
...
After step 4, permissions show:

rwxrwxrwx

After step 5, owner and group both show:
...
You are setting up a new Node.js REST API from scratch and publishing it to GitHub.
...
GET /health → returns HTTP status 200 with JSON response:
...
Running the server locally and calling:
